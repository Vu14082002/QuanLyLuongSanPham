            ...
            for (PhanCongCongNhan e : listPhanCong) {
                if (e.getCongDoan().getSanPham().getMaSanPham().equals(sp.getMaSanPham())) {
                    modelPhanCong.addRow(new Object[]{e.getMaPhanCong(), e.getCongDoan().getSanPham().getMaSanPham(),
                        e.getCongNhan().getMaCongNhan(), e.getCongDoan().getMaCongDoan(), e.getCongDoan().getTenCongDoan(),
...
//        }
    ...
    private void initComponents() {
        ...
        tblPhanCong.setModel(new javax.swing.table.DefaultTableModel(
                ...
                // new code
                new String[]{
                    "Mã phân công", "Mã sản phẩm", "Mã công nhân", "Mã công đoạn", "Tên  công đoạn", "Tổ/Nhóm", "Số lượng cần làm", "Ngày phân công"
                }
        ) {
            boolean[] canEdit = new boolean[]{
    ...
    private void btnLuuActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        if (checkSoLuongCanLam()) {
            // new code
            int soLuongPhanCong = Integer.parseInt(txtSoLuongCanLam.getText());
            CongNhan_DAO congNhanDao = new CongNhan_DAO();
            ArrayList<CongNhan> congNhanList1 = congNhanDao.layDanhSachCongNhanTheoMaTo(toNhom.getMaToNhom());
            int soLuongCongNhan = 0;
            if (congNhanList1 != null) {
                soLuongCongNhan = congNhanList1.size();
            }
            int soLuongLamCuaMotCongNhan = soLuongPhanCong / soLuongCongNhan;
            int soLuongDu = soLuongPhanCong - soLuongLamCuaMotCongNhan * soLuongCongNhan;
            ...
            if (checkPhanCong) {
                ...
                if (daoPhanCong.checkDuocPhanCong(maCongDoan, toNhom.getMaToNhom())) {
                    for (CongNhan congNhanTheoToNhom : congNhanList) {
                        int soLuongPhaiLam = soLuongLamCuaMotCongNhan;
                        if (index == 0) {
                            soLuongPhaiLam += soLuongDu;
                            index++;
                        }
...
//                        if (checkSoLuongCanLam()) {
                        ...
                        PhanCongCongNhan phanCong = new PhanCongCongNhan(maPhanCong, congNhanTheoToNhom, congDoan, nhanVienChamCong, ngayPhanCong, soLuongPhaiLam, toNhom);
...
//                        }
                ...
                int soLuongPhaiLam = Integer.parseInt(txtSoLuongCanLam.getText());
//                if (index == 0) {
//                    soLuongPhaiLam += soLuongDu;
//                    index++;
//                }
                String maNhanVien = tblPhanCong.getValueAt(tblPhanCong.getSelectedRow(), 2).toString();
//                daoPhanCong.suaMotPhanCongNhanTheoMaCongDoan(toNhom.getMaToNhom(), soLuongPhaiLam, cmbMaCongDoan.getSelectedItem().toString(), maNhanVien);
                if (daoPhanCong.suaMotPhanCongNhanTheoMaCongDoan(toNhom.getMaToNhom(), soLuongPhaiLam, cmbMaCongDoan.getSelectedItem().toString(), maNhanVien)) {
                    taiDuLieuLenBangPhanCong();
